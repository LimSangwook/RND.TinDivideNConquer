...
*/
static int in_circle2( point2d_t *pt0, point2d_t *pt1, point2d_t *pt2, point2d_t *p )
{
	mat3_t	ma, mbx, mby, mc;
	real	x0y0, x1y1, x2y2;
	real	a, bx, by, c, res;
...
static int del_init_tri( delaunay_t *del, int start )
{
	...
	} else if( classify_point_seg(pt0, pt2, pt1) == ON_RIGHT )/* 2nd case */
	{
		/* set halfedges points */
		d0->vertex	= pt0;
		d1->vertex	= pt1;
		...
		del->rightmost_he	= d2;
		del->leftmost_he		= d0;
	}else if( classify_point_seg(pt0, pt2, pt1) == ON_SEG )
	{
		halfedge_free(d4);
		halfedge_free(d5);
		/* set halfedges points */
